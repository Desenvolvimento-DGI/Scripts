...
cd ${ORIGEM}
COORDENADAS="`\ls -1 | grep -i LO8${PARORBITA}`"
...
do
		...
		IMGWARP="`echo ${IMG1} | cut -f 1 -d .`_WARP.TIF"
		...
		convert -brightness-contrast 20x15 ${PNGTRANSPARENTE}  ${PNGTRANSPARENTEBC}
		...
		echo ""
		echo "Compactando todas as bandas em formato ZIP ..."
        ...
        ${CMDGZIP} -f -v -S .zip *_B10.TIF
        ${CMDGZIP} -f -v -S .zip *_B11.TIF

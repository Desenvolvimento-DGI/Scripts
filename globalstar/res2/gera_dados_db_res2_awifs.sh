...
ARQLOGS="/home/cdsr/res2/logs/res2awifs_${PARANO}${PARMES}${PARDIA}.log"
ARQEMAILCQ="/home/cdsr/res2/EmailCQ_${PARANO}${PARMES}${PARDIA}_.log"


CMDMYSQL='/usr/bin/mysql --login-path=catalogo-gerente '

ARQUIVOSQLDELETE="/home/cdsr/res2/DELETEAWIFS_${PARANO}${PARMES}${PARDIA}.sql"
ARQUIVOSQL="/home/cdsr/res2/INSERTRES2AWIFS_${PARANO}${PARMES}${PARDIA}.sql"
ARQUIVOSCENERES2="/home/cdsr/res2/INSERTSCENERES2AWIFS_${PARANO}${PARMES}${PARDIA}.sql"

QLORIGEM="/QUICKLOOK/RESOURCESAT2/AWIF/"
	...
	do
		...
		do
			...
			do
				...
				echo "COORDENADAATUAL ==>  ${COORDENADAATUAl}"
				cd "${ORIGEM}${AMATUAl}${PERIODOATUAl}${COORDENADAATUAl}/${QUADRANTEATUAL}"
				pwd
				...
				if [ -e "${ORIGEM}${AMATUAl}${PERIODOATUAl}processado.lock" ]
				then				
					echo ""
					echo "Arquivo ja processado"
					echo "${AGORA} ${AMATUAl} - ${PERIODOATUAl} - ${COORDENADAATUAl}- ARQUIVO ${NOMEARQUIVO} JÁPROCESSADO" >> ${ARQLOGS}
				...
				AGORA=`date +"%Y/%m/%d %H:%M:%S"`
				echo "${AGORA} ${AMATUAl} - ${PERIODOATUAl} - ${NOMEARQUIVO} - OK" >> ${ARQLOGS}
				...
				SQLCAMPOS="${SQLCAMPOS} Quadrante, Deleted, Catalogo, CriadoEm ) "
				...
				SQLVALORES="${SQLVALORES} '${QUADRANTEATUAL}', ${DELETED}, ${CATALOGO}, '${CRIADOEM}' );"
				...
				DRD="${SATELITE}_${MISSAO}_${SENSOR}_${ANO}${MES}${DIA}_${ORBITA}_${PONTO}_${QUADRANTEATUAL}"
				GRALHA="${DRD}"

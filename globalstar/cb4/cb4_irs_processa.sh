...
DESTINOQL='/QUICKLOOK/CBERS4/IRS/'
...
ARQUIVOSQLDELETE="${HOMESCRIPT}DELETECB4IRS_${NUMPPID}.sql"
ARQUIVOSQL="${HOMESCRIPT}INSERTCB4IRS_${NUMPPID}.sql"
ARQUIVOSQLSCENE="${HOMESCRIPT}INSERTSCENECB4IRS_${NUMPPID}.sql"
LOGARQUIVOSCORROMPIDOS="${HOMESCRIPT}IRS-ARQUIVOS-CORROMPIDOS-${NUMPPID}.log"
...
do
	...
	DIRPERIODOS="`\ls -1r | grep ^CBERS4_IRS`"
	...
	do
		...
		do
			...
			TIFBANDAS342GEO8="${TIFNOMEPADRAO}-342-GEO-8BITS.tif"
			TIFBANDAS342GEO8C="${TIFNOMEPADRAO}-342-GEO-8BITS-CONTRAST.tif"
			...
			rm -fv ${TIFBANDAS342} ${TIFBANDAS342GEO}
			rm -fv ${PNGMIN} ${PNGPEQ} ${PNGMED} ${PNGGRD}
			...
			then
				...
				echo ""
				echo "Convertendo imagem de 16 bits para 8 bits ..."
				echo ""
				${PATHGDAL}gdal_translate -of GTIFF -ot Byte -scale ${TIFBANDAS342GEO}  ${TIFBANDAS342GEO8}


				echo ""
				echo "Aplicando ajuste de contraste na imagem de 8 bits ..."
				echo ""
				convert -contrast ${TIFBANDAS342GEO8} ${TIFBANDAS342GEO8C}
				...
				${PATHGDAL}gdal_translate -of PNG -a_nodata 0  -outsize ${LARGURAQL} ${ALTURAQL} ${TIFBANDAS342GEO8C} ${PNGMIN}

				# Quicklook com resolucao pequena de pixels 
				${PATHGDAL}gdal_translate -of PNG -a_nodata 0  -outsize ${PERCPEQ} ${PERCPEQ} ${TIFBANDAS342GEO8C} ${PNGPEQ}

				# Quicklook com resolucao media de pixels 
				${PATHGDAL}gdal_translate -of PNG -a_nodata 0  -outsize ${PERCMED} ${PERCMED} ${TIFBANDAS342GEO8C} ${PNGMED}

				# Quicklook com resolucao grande de pixels 
				${PATHGDAL}gdal_translate -of PNG -a_nodata 0  -outsize ${PERCGRD} ${PERCGRD} ${TIFBANDAS342GEO8C} ${PNGGRD}
				...
				rm -fv ${TIFBANDAS342} ${TIFBANDAS342GEO} ${TIFBANDAS342GEO8} ${TIFBANDAS342GEO8C}
				rm -fv *.aux.xml *.png.xml 

				
				echo ""
				echo "Copiando quicklooks para /QUICKLOOK/CBERS4/IRS ..."
				echo ""

...
# diretorio de saida #
dir_out='/GRALHA/RECUPERACAO_DE_DADOS/Dados_NOAA_renomeados_novissimo_02'
...
if [ $ano_mes ] && [ $satelite ] && [ $missao ]
then
	# Diretorio de pesquisa #
	dir_hrpt="/L0_"$satelite"/"$ano_mes"/HRPT"
	cd $dir_hrpt
	ls 'NOAA_'$missao'_HRPT_C'*  > $dir_exe'/lista_nome_novo_'$ano_mes'_'$satelite
	ls 'NOAA_'$missao'_HRPT_N'*  >> $dir_exe'/lista_nome_novo_'$ano_mes'_'$satelite

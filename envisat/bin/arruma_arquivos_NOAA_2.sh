#!/bin/bash -x

# Script que arruma arquivo renomeados NOAA #
dir_exe="/home/cdsr/bin"
dir_arquivo="/L0_NOAA12"
dir_saida="/GRALHA/RECUPERACAO_DE_DADOS/DADOS_NOAA12"

if [ $1 ]
then
	ano_mes=$1

	dir_arquivo=$dir_arquivo"/"$ano_mes"/HRPT/"
	
	# Lista dos arquivos #
	cd $dir_arquivo
	ls *CP1* > $dir_exe"/lista_arq2_"$ano_mes
	cd $dir_exe
	while read arquivos
	do
		echo $arquivos
		cp -rf $dir_arquivo"/"$arquivos $dir_exe
		
		# descompacta #
		tar zxf $arquivos
		
		# nome da pasta #
		pasta1=`echo $arquivos | cut -d "." -f1`
		pasta2=`echo $arquivos | cut -d "." -f2`
		pasta=$pasta1"."$pasta2
		
		if [ -e $arquivos ]
		then
			rm -rf $arquivos
		fi
		
		cd $pasta
		# vexifica se existe o conjunto de arquivos #
		ls > $dir_exe'/lista_interna1_'$ano_mes
		cd $dir_exe
		
		
		cd $pasta
		# vexifica se existe o conjunto de arquivos #
		ls > $dir_exe'/lista_interna2_'$ano_mes
		cd $dir_exe
		while read internos
		do
			tipo=`echo $internos | cut -d "." -f3`
			if [ $tipo == 'qlk' ] || [ $tipo == 'QLK' ]
			then
				cd $pasta
				mv $internos $pasta".png"
				cd $dir_exe

			else
				novo_tipo=`echo $tipo | tr '[A-Z]' '[a-z]']`
				cd $pasta
				mv $internos $pasta"."$novo_tipo
				cd $dir_exe
			fi
		done < $dir_exe'/lista_interna2_'$ano_mes
		
		# compacta #
		tar cf $pasta".tar" $pasta
		gzip $pasta".tar"
		# move para diretorio de saida #
		cd $dir_saida
		mkdir -p $ano_mes
		cd $dir_exe
		mv $pasta".tar.gz" $dir_saida"/"$ano_mes
		
		# removendo a pasta #
		cd $dir_exe
		if [ -e $pasta ]
		then
			rm -rf $pasta
		fi
		
		
		
		
		
	done < $dir_exe"/lista_arq2_"$ano_mes
fi
exit

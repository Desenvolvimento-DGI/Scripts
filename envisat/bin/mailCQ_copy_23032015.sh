...
data=`date '+%Y-%m-%d'`
# AQUA #
pesq_aqua=`mysql -u gerente -pgerente.200408 -D catalogo -s -e "SELECT SceneId, Date, Satellite FROM Scene WHERE OperatorId = '' 
...
then
	...
	then
		rm emailAQUA
		up_aqua=`mysql -u gerente -pgerente.200408 -D catalogo -s -e "UPDATE Scene SET OperatorId = 'vazio' WHERE Satellite = 'A1' AND SceneId = '$sceneid_aqua'"`
	fi
fi
# TERRA #
pesq_terra=`mysql -u gerente -pgerente.200408 -D catalogo -s -e "SELECT SceneId, Date, Satellite FROM Scene WHERE OperatorId = '' 
...
then
	...
	then
		rm emailTERRA
		up_terra=`mysql -u gerente -pgerente.200408 -D catalogo -s -e "UPDATE Scene SET OperatorId = 'vazio' WHERE Satellite = 'T1' AND SceneId = '$sceneid_terra'"`
	fi
fi
# NPP #
pesq_npp=`mysql -u gerente -pgerente.200408 -D catalogo -s -e "SELECT SceneId, Date, Satellite FROM Scene WHERE OperatorId = '' 
...
then
	...
	then
		rm emailNPP
		up_npp=`mysql -u gerente -pgerente.200408 -D catalogo -s -e "UPDATE Scene SET OperatorId = 'vazio' WHERE Satellite = 'NPP' AND SceneId = '$sceneid_npp'"`
	fi
fi

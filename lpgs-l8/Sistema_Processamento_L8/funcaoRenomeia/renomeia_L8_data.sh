...
cd $dir_md'/'$ano'_'$mes
ls *$ano'_'$mes'_'$dia*'.tar.gz' > $dir_exe'/.listaMD'
cd $dir_exe
...
do
	...
	cd $nome_pasta
	ls 'OLI-RT/' > $dir_exe'/.listaOLI'
	#ls 'TIRS-RT/' > $dir_exe'/.listaTIRS'
	...
	# Grear lista dos arquivos pata TIRS e 
	# captura o ROOT ID do arquivo para o TIRS
	ls 'TIRS-RT/' > $dir_exe'/.listaTIRS'
	cd $dir_exe
	root_id_arquivo_tirs=`cat $dir_exe'/.listaTIRS' | head -1`
	root_id_arquivo_tirs=`echo $root_id_arquivo_tirs | cut -d "." -f1`

	pesquisa_root_id_tirs="${root_id_arquivo_tirs}*"
	if [ "${root_id_arquivo_tirs}" == "" ]
	then
		pesquisa_root_id_tirs=""
		prefixo_sensor='LO8'
	else
		prefixo_sensor='LC8'
	fi
	...
	# Arquivos do TIRS, caso existam
	cd $dir_exe
	cd $nome_pasta

	mv RT-TIRS/* $nome_novo_l8
        rm -fr RT-TIRS
	...
	md5sum -b $root_id_arquivo* $pesquisa_root_id_tirs > $nome_novo_l8'_MD5.txt'

...
then
	...
	then
		...
		then
			...
			proc_status = 'ERRO-MD5' WHERE proc_caracter_chave = '$caracter_chave'"`
		fi
		# IDF #
		if [ $IDF == "ERRO" ]
		then
			up_renomeia=`mysql -h 150.163.134.104 -P3333 -u dgi -pdgi.2013 -D sistema_L8 -s -e "UPDATE processos_Landsat8 SET proc_data_final = '$data_final', 
			proc_status = 'ERRO-IDF' WHERE proc_caracter_chave = '$caracter_chave'"`
		fi
		# MD5 e IDF #
		if [ $MD5 == "ERRO" ] && [ $IDF == "ERRO" ]
		then
			up_renomeia=`mysql -h 150.163.134.104 -P3333 -u dgi -pdgi.2013 -D sistema_L8 -s -e "UPDATE processos_Landsat8 SET proc_data_final = '$data_final', 
			proc_status = 'ERRO-MD5-IDF' WHERE proc_caracter_chave = '$caracter_chave'"`
		fi
	else
		up_renomeia=`mysql -h 150.163.134.104 -P3333 -u dgi -pdgi.2013 -D sistema_L8 -s -e "UPDATE processos_Landsat8 SET proc_data_final = '$data_final', 
		proc_status = 'FINALIZADO' WHERE proc_caracter_chave = '$caracter_chave'"`
	fi
	...
	# Atualiza os dados no banco de dados #
	up_renomeia=`mysql -h 150.163.134.104 -P3333 -u dgi -pdgi.2013 -D sistema_L8 -s -e "UPDATE processos_Landsat8 SET proc_data_final = '$data_final', 
	proc_status = 'ERRO-ROOT_ID' WHERE proc_caracter_chave = '$caracter_chave'"`

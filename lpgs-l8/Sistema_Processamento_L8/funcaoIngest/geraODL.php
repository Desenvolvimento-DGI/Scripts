...
if ( $_CONTROLE_PROCESSOS == 0  ||  $_CONTROLE_PROCESSOS == 1 )
{
	...
	// 1.3 - Valida e altera o nome da variavel CPF //
	...
	$INPUT_PATH = "/dados/L0_LANDSAT8/MISSION_DATA/".$ano_mes."/";
	// consistencia - cria o diretorio caso ele nao exista //
	system("mkdir -p $INPUT_PATH");
	// 1.5 - Variavel que contem o nome do L0R_OUTPUT_PATH //
	$L0R_OUTPUT_PATH = "/dados/L0_LANDSAT8/L0Ra/".$ano_mes."/".$passagem."/" ;
	...
	// 2 - Criar arquivos ODL e os armazena no local correto //
	// 2.1 - Cria a pasta onde sera armazenado os arquivos ODL //
	$dir_ODL = "/dados/JPF/";
	//--echo $passagem;
	system("cd $dir_ODL; mkdir ${passagem}");
	// nome da pasta final do ODL //
	$dir_ODL_final = $dir_ODL.$passagem;


	// Definicao do sensor conforme o passagem
	// Necessario para gerar os potos corretamente e para ser processado corretamente pelo
	// processo SUBSETTER
	// 

	//$arquivo_auxiliar = fopen("/dados/htdocs/Sistema_Processamento_L8/funcaoIngest/tipo_sensor.txt", "a");


	$composicao_passagem=strtoupper(substr($passagem, 0, 2));

	if ( $composicao_passagem == "LC" )
	{
		$definicao_sensor='SENSOR_ID ="OLI_TIRS"';
	}
	else
	{
		$definicao_sensor='SENSOR_ID ="OLI"';
	}

	//fwrite($arquivo_auxiliar, $passagem . "\n");
	//fwrite($arquivo_auxiliar, $composicao_passagem . "\n");
	//fwrite($arquivo_auxiliar, $definicao_sensor . "\n\n\n");
	...
	// 2.2 - Cria os arquivo ODL apartir das variaveis anteriormente capturadas //
	// 2.2.1 - Loop de todos os ponto //
	for ($ponto = $valor_inicial_loop; $ponto <= $valor_final_loop; $ponto++) 
	{
		// 2.2.2 - variaveis que irao ser inseridas no template ODL //
		if ( $ponto < 100 )
		{
			$ponto_odl = "0".$ponto;
		}	else	{
		...
		// 2.2.3 - Variavel que contem o valor do L0R_INTERVAL_ID //
		$L0R_INTERVAL_ID = $parte_passagem_prefixo.$base.$ponto_odl.$parte_passagem_sufixo;
		$nome_gerar_arquivo_ODL = $L0R_INTERVAL_ID;
		// 2.2.4 - Alteral o valor da variavel que contem o valor do L0R_TEMP_PATH //
		$L0R_TEMP_PATH_ODL = $L0R_TEMP_PATH.$L0R_INTERVAL_ID;
		// 2.2.5 - Alteral o valor da variavel que contem o valor do L0R_WORK_PATH //
		...
		while($_LINHA_Template = mysql_fetch_array($lista_arquivo_ODL_BD))
		{
			...
			fwrite($arquivo_ODL, $definicao_sensor . "\n");
		...
		// 2.4 - Move o arquivo para a pasta correta //
		//--echo "mv ${nome_gerar_arquivo_ODL}.odl $dir_ODL_final <br />";
		system("mv ${nome_gerar_arquivo_ODL}.odl $dir_ODL_final");
	}
	...
	echo "
	<script type='text/javascript'>
		alert('Aguarde os processos INGEST em execução terminarem!');
		window.location.href = '../home/listaDeProcessos.php?';
	</script>
";
}
?>
